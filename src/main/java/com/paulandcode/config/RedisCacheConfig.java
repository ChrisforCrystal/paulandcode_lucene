...
public class RedisCacheConfig extends CachingConfigurerSupport {
	/** 默认时间缓存, 单位: 秒. **/
	@Value("${spring.redis.default-cache-time}")
	private int defaultCacheTime;
	
	/** 短时间缓存, 单位: 秒. **/
	@Value("${spring.redis.short-cache-time}")
	private int shortCacheTime;
	
	/** 长时间缓存, 单位: 秒. **/
	@Value("${spring.redis.long-cache-time}")
    ...
    public CacheManager cacheManager(RedisConnectionFactory redisConnectionFactory) {
        return new RedisCacheManager(
				...
				// 默认缓存时间策略
                this.getRedisCacheConfigurationWithTtl(defaultCacheTime),
				// 指定缓存时间策略
	 ...
	 * 		@Cacheable(value = "short_cache") // 缓存时间: shortCacheTime
     *		@Cacheable(value = "long_cache") // 缓存时间: longCacheTime
     *		@Cacheable(value = "others") // 默认缓存时间: defaultCacheTime
     *		还可以根据需要再增加缓存时间策略
	 * @return
	 */
    private Map<String, RedisCacheConfiguration> getRedisCacheConfigurationMap() {
        Map<String, RedisCacheConfiguration> redisCacheConfigurationMap = new HashMap<>(2);
        redisCacheConfigurationMap.put("short_cache", this.getRedisCacheConfigurationWithTtl(shortCacheTime));
        redisCacheConfigurationMap.put("long_cache", this.getRedisCacheConfigurationWithTtl(longCacheTime));
